...
This section presents the results of Experiment 2, which evaluates monte carlo dropout (MC Dropout) and deep ensembles as uncertainty estimation methods applied to the best performing regularized RGCN model from Experiment 1 (edge dropout of 0.2 and label smoothing of 0.1). The performance and uncertainty estimation metrics are compared against the baseline regularized model without uncertainty estimation.

\subsection{Link Prediction Performance}
\label{subsec:link_prediction_performance_exp2}
Table \ref{tab:link_prediction_performance_exp2} summarizes the link prediction performance of the uncertainty estimation methods on the WN18RR and FB15k-237 datasets.

\begin{table}[htbp]
        \centering
        \includegraphics[width=0.75\textwidth]{figures/experiment2/table}
