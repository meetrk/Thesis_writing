...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{figures/experiment/model}
...
\subsection{Experiment 1: Regularization Impact}
...
\subsection{Experiment 2: Uncertainty Estimation Methods}
...
\newline
\textbf{Methodology:}
We will use the model from \ref{subsec:uncertainty_estimation_methods} and apply the calibration methods from \ref{subsec:calibration_methods}. We will evaluate MonteCarlo Dropout + Temperature Scaling, MonteCarlo Dropout + Platt Scaling and Monte Carlo Dropout + Isotonic Regression \& Deep Ensembles + Temperature Scaling, Deep Ensembles + Platt Scaling and Deep Ensembles + Isotonic Regression. We will assess the performance of these hybrid models on the test set using the same metrics as before to determine the effectiveness of combining these techniques.