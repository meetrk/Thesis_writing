...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/approach/messagepassing}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{figures/approach/pipeline}
    \caption{Figure representing Autoencoder Architecture}
    \label{fig:pipeline}
\end{figure}
...
In the parameteric approaches,Platt Scaling and its simplar adaption Temperature Scaling are used to calibrate the predictions. These methods involve learning a single or multiple parameters to scale the logits obtained from the model before applying softmax or sigmoid function.



\subsubsection{Platt Scaling}
\label{subsubsec:platt_scaling}
The Platt Scaling is a method proposed by (Platt 1999)\cite{platt1999probabilistic} to calibrate the predictions of SVM model with use of logistic regression. In this method, we learn two parameters \begin{math}A\end{math} and \begin{math}B\end{math} to scale the logits obtained from the model. Given the logits \begin{math} s = f(e_h, r, e_t) \end{math} from equation \ref{eq:distmult_scoring}, we scale the logits as:
\begin{align}
    p_{calibrated} = \sigma(As + B) \label{eq:platt_scaling}
\end{align}. For calibration, We always use validation set to learn the optimal values for parameters \begin{math}A\end{math} and \begin{math}B\end{math} by minimising the loss. We have used Negative Log Likelihood (NLL) in combination with Margin ranking loss (MRL) to preserve the ranking order of the logits. 
\begin{align}
    \mathcal{L} = \mathcal{L}_{NLL}(Platt(s_{+,-}), y) + \lambda * \mathcal{L}_{MRL}(Platt(s_{+}),Platt(s_{-})) \label{eq:platt_loss}
    \end{align} 
Here \begin{math}s_{+}\end{math} and \begin{math}s_{-}\end{math} are the logits for positive and negative triples respectively and \begin{math}y\end{math} are the true labels. \begin{math}\lambda\end{math} is the hyperparameter to control the weightage of MRL loss. In our implementation, we set \begin{math}\lambda = 1\end{math}. We used LBFGS optimizer to learn the parameters \begin{math}A\end{math} and \begin{math}B\end{math}. The advantage of this method is that it is post-hoc and does not require retraining of the model. 
...
\end{align} We use validation set to learn the optimal value for parameter \begin{math} T \end{math} by minimising the loss same as \ref{eq:platt_loss} between the calibrated probabilities and true labels. Once we learned the optimal value of \begin{math} T \end{math}, we can use it to calibrate the predictions on test set. If the value of the \begin{math} T \end{math} is greater than 1, it indicates that the model is overconfident and underconfident when the value is less than 1. 
...
% what about the hybrid approaches? We can put that in the experiments setting as we have already expalined the both of them seperately in detail. We 